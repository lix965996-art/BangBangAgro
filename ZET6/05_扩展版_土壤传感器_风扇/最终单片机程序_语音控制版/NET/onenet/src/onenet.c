...
#define PROID			"yN8ZASFo9d"

#define ACCESS_KEY		"o8FzlxdX8UO7UfvRKduJdztLTalf0Yvn25vH2afpA/k="
...
_Bool OneNET_RegisterDevice(void)
{
	...
	OneNET_Authorization("2025-05-01", PROID, 1956499200, ACCESS_KEY, NULL,
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"soil\":{\"value\":%d},", soil_percent);
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"led\":{\"value\":%s},", led_info.Led_Status ? "true" : "false");
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"fan\":{\"value\":%s},", fan_info.Fan_Status ? "true" : "false");
	strcat(buf, text);
	memset(text, 0, sizeof(text));
